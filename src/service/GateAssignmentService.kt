...
class GateAssignmentService {
    ...
    suspend fun allocateGateForFlights(flightsModel: FlightsModel): ResponseModel {
        ...
        val time = measureTimeMillis {
            ...
            var averageWaitTime = runBlocking {
                async { getAverageTime(arrivalWithFlight, departureWithFlight) }
            }

            responseModel.averageUseOfGateInSeconds = averageWaitTime.await()
            //responseModel.averageUseOfGateInSeconds = getAverageTime(arrivalWithFlight, departureWithFlight)
        }
        println(time)
    ...
    private fun getAverageTime(
        arrivalWithFlight: HashMap<String, Long>,
        departureWithFlight: HashMap<String, Long>
    ): Long {
